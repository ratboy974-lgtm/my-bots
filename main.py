...
# --- 1. SERVER HEALTH CHECK (Per tenere vivo il bot su Render) ---
app = Flask(__name__)
@app.route('/')
...
def genera_e_invia_foto(bot, chat_id):
    try:
        ...
        # Prompt specifico per l'estetica di Luna
        ...
        with open(fname, "rb") as photo:
            bot.send_photo(chat_id, photo, caption="For you, babe... 😉", timeout=60)
        os.remove(fname)
    except:
        bot.send_message(chat_id, "I'm a bit shy right now, ask me again! 🌊")
...
if bot_luna:
    ...
    def handle_luna(m):
        ...
        # Gestione richiesta Foto
        if any(p in msg_low for p in ["foto", "selfie", "pic", "photo"]):
            bot_luna.send_message(chat_id, "Wait a second, I'm posing for you... 📸")
            genera_e_invia_foto(bot_luna, chat_id)
            return

        # Personalità Uncensored
        p_luna = (
        ...
        try:
            ...
            # Luna risponde sempre con un vocale
