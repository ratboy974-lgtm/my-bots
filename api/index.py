...
# Chat ID autorizzato (solo tu potrai usare il bot)
...
def handle_msg(m):
    ...
    # Estrazione testo del messaggio
    user_text = ""
    ...
    try:
        # Chiamata ad OpenRouter con modello valido
        response = client.chat.completions.create(
            model="google/gemini-2.0-flash",
            messages=[
                {
                    "role": "system", 
                    "content": "Sei Lora, un'assistente IA amichevole, precisa ed empatica. Rispondi sempre in modo chiaro e utile."
                },
...
# Gestione delle rotte Webhook universali per Vercel
...
def handle_webhook(path=""):
    if request.method == 'POST':
        try:
            ...
            print(f"Errore webhook Lora: {e}")
        return "!", 200
    return "Lora V100 is Active! 🚀", 200
